...
function registerCommands(context, api, auth, fileSync) {
    ...
    const generateLinkCommand = vscode.commands.registerCommand('codeShareLink.generateLink', async () => {
        try {
            ...
            await vscode.window.withProgress({
                ...
                const selectedAction = await vscode.window.showInformationMessage(`Shareable link generated: ${link}`, copyAction, manageAction);
    ...
    const openSharedProjectCommand = vscode.commands.registerCommand('codeShareLink.openSharedProject', async () => {
        try {
            ...
            const linkOrPicker = await vscode.window.showQuickPick(['Enter a shared link', 'Select from shared projects'], { placeHolder: 'How would you like to open a shared project?' });
            if (!linkOrPicker) {
                return;
            }
            let projectId;
            ...
            await vscode.window.withProgress({
                ...
                const openInCurrentWindow = await vscode.window.showInformationMessage(`Project downloaded to ${projectFolderUri.fsPath}. Open in new window?`, 'Yes', 'No');
